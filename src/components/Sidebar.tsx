...
const Sidebar = ({ chapter, year, focusRef }: SidebarProps) => {
  ...
  // const retreatYears = chapterId.isSuccess ?
  //   trpc.retreat.getRetreatYears.useQuery(chapterId.data ?? "")
  //   : null;
  const retreatYears = trpc.retreat.getRetreatYears.useQuery(
    chapterId.data ?? "",
  );

  const archiveOption: OptionType = {value: "Add Archive", label: "Add Archive"};
  ...
  const getOptionFromYear = useCallback(
    (yr: number) => {
      return (
        options.find((o) => o.value === yr.toString()) ?? archiveOption
      );
    },
    [options],
  );
  ...
  useEffect(() => {
    setSelectedOption(getOptionFromYear(year));
  }, [getOptionFromYear, year]);
