import { Text, Flex, Spacer, Box, IconButton, Button } from "@chakra-ui/react";
...
const ChapterCard = ({ chapter }: ChapterCardProps) => {
  ...
  return (
    <Box
      ...
      onClick={(event) => window.location.href = `/chapters/${chapter.id}`}>
      <Flex mt="2" mr="2">
        <Spacer />
        <IconButton
          aria-label="settings"
          ...
          height="40px"
          width="40px"
          icon={<BiSolidEdit size="20px" onClick={stopPropagation} />}
        />
        <NewChapterModal
          isOpen={isOpenAddChapterModal}
          onClose={onCloseAddChapterModal}
